...
- `[DONE]` Latest completed phase before this checklist update: Phase 62.
...
- `[DONE]` Add launch readiness storage-driver gate so strict launch checks fail until `content_storage.driver=mysql`.
- `[WAITING]` Run `SITE/scripts/check-mysql-smoke.php` on the production host or a real dev MySQL database.
- `[WAITING]` Production must switch from local/demo JSON storage to MySQL; strict readiness now gates this with `config.storage_driver`.
...
- `[DONE]` Dedicated new-session Codex continuation prompt exists at `docs/codex-continuation-prompt.md`.
...
51. [DONE] Phase 62: Dedicated Codex continuation prompt.
